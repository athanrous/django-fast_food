...
from places.models import Restaurant, Town 
...
def choose_town(request, food_id):
    ...
    return render(request, 'places/choose_town.html', {'food_id':food_id, 'town_list':town_list})

def choose_restaurant(request,food_id,town_id):
    rests = Restaurant.objects.filter(food__id=food_id, town__id=town_id)
    return render(request, 'places/choose_restaurant.html', {'rests':rests})
...
def restaurant(request, rest_id, message=''):
    ...
    return render(request, 'places/restaurant.html', {'restaurant':rest, 'rest_menu':rest_menu, 'message':message})
